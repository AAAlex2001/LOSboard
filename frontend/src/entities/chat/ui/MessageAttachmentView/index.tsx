...
export const MessageAttachmentView = ({
  ...
  const needsBlob = kind === "image" || kind === "video";
  const [docDownloading, setDocDownloading] = useState(false);
  const [lightboxOpen, setLightboxOpen] = useState(false);
  const { src, error } = useSecureAsset(needsBlob ? url : null);
  ...
  if (kind === "image") {
    ...
    return (
      <>
        ...
        >
          <img src={src} alt={filename} className={style.image} />
        </button>
        {lightboxOpen && (
          <Lightbox
            src={src}
  ...
  if (kind === "video") {
    if (error) return <span className={style.error}>{error}</span>;
    if (!src) return <div className={style.videoSkeleton} />;
    return (
      <>
        <button
          type="button"
