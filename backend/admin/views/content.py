...
class ContentPageAdmin(AdminOnly, ModelView, model=ContentPage):
    ...
    form_args = {
        "slug": {
            "description": (
                "Латиница, без пробелов. Slug определяет где появится блок:\n"
                "• `contacts` — страница «Связаться с нами» (заголовок и интро)\n"
                "• `contacts-data` — блок с контактами на странице «Связаться с нами»\n"
                "• `contacts-hours` — блок с графиком работы\n"
                "• `pricing` — основная таблица тарифов на странице «Реклама в LOS»\n"
                "• `pricing-promo` — промо-баннер на странице рекламы\n"
                "• `pricing-tech-board` / `-social` / `-mobile` / `-tour` — блоки технических характеристик\n"
                "• Остальные slug'и доступны по URL `/docs/<slug>`."
            ),
        },
        "title": {
            "description": "Внутреннее имя блока. Видно в списке справа и помогает находить нужный блок.",
        },
        "is_published": {
            "description": "Если выключено — блок не отображается на сайте, но остаётся в базе.",
        },
        "body": {
            "description": (
                "Содержимое блока. Поддерживаются:\n"
                "— Заголовки (H2/H3), списки, цитаты (blockquote), ссылки, выделение.\n"
                "— Специальные маркеры (пиши каждый в отдельном абзаце):\n"
                "  [BANNER 970x90 ТОП баннер] — макет рекламного баннера с размером и подписью\n"
                "  [BANNER 970x90 ТОП баннер; примечание под банером] — то же с примечанием\n"
                "  [CARD 400x218 Карусель] — карточка-превью\n"
                "  [PHONE 402x874 Главный экран] — макет телефона\n"
                "  [NOTE текст подсказки] — синяя подсказка по центру\n"
                "  [HINT текст подсказки] — серая подсказка по центру\n"
                "  [INFO Заголовок карточки]\n"
                "  …содержимое карточки…\n"
                "  [/INFO] — карточка с заголовком и телом (несколько подряд встанут в сетку)\n"
                "  [INFO! Заголовок] … [/INFO] — выделенная (синяя) карточка."
            ),
        },
    }
